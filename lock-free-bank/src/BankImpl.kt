...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    ...
    override fun withdraw(index: Int, amount: Long): Long {
        ...
        require(amount > 0) { "Invalid amount: $amount" }
        while (true) {
            val oldAccount = account(index)

            if (oldAccount.invokeOperation()) {
                continue
            }

            check(oldAccount.amount - amount >= 0) { "Underflow" }
            val newAccount = Account(oldAccount.amount - amount)

            if (accounts[index].compareAndSet(oldAccount, newAccount)) {
    ...
    private fun acquire(index: Int, op: Op): AcquiredAccount? {
        ...
        while (true) {
            ...
            if (oldAccount is AcquiredAccount && oldAccount.op == op) {
                return oldAccount
            }
            ...
            val newAccount = AcquiredAccount(oldAccount.amount, op)
            if (accounts[index].compareAndSet(oldAccount, newAccount)) {
                return newAccount
            }
        }
    ...
    private inner class TransferOp(val fromIndex: Int, val toIndex: Int, val amount: Long) : Op() {
        ...
        override fun invokeOperation() {
            ...
            try {
                ...
                acquire(max(fromIndex, toIndex), this)
